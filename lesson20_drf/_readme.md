...
**Django REST Framework (DRF)** — это **дополнение** к Django, которое помогает создавать **API** (интерфейсы для обмена данными между системами).

> То есть: DRF — не «встроенная» часть Django, а **отдельная библиотека**, которую ты **устанавливаешь через pip** (`pip install djangorestframework`) и подключаешь в проект.

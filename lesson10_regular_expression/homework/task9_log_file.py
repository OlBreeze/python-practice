...
def analyze_log_file(file_path):
    ...
    # defaultdict — це майже те саме, що звичайний dict, але якщо ти звертаєшся до ключа, якого ще нема,
    # він створює його автоматично із типовим значенням, яке ти задаєш.
    # У випадку defaultdict(int), типове значення — це 0, тому що int() повертає 0.
    try:
        with open(file_path, 'r') as log_file:
            for line in log_file:
                # parts = line.split() # якщо в лог файл починаеться з IP адреси
                # if parts:
                #     ip = parts[0]
                #     ip_counts[ip] += 1
        ...
        # sorted(..., key=lambda x: x[1], reverse=True) : сортируеm эти пары по второму элементу кортежа, то есть по количеству запросов (а не по IP)
